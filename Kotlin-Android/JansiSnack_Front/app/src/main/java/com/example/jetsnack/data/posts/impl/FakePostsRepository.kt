...
//    private val favorites = MutableStateFlow<Set<String>>(setOf())
    ...
    override fun observeFavorites(): Flow<Set<String>> = favorites.map { posts -> posts.map { post -> post.id }.toSet() }
    override fun observePostsFeed(): Flow<PostsFeed?> = postsFeed

    override fun toggleFavorite(postId: String) {
        favorites.update { it ->
            it.addOrRemove(postsList.first { it.id == postId })
        }
